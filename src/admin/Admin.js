    ...
    if(this.state.message === ''){
      ...
      success : data => {	 				
        if(data.request === 'rejected'){
          toastr.info('La petición ha sido rechazada');
          this.setState({
  ...
  acceptRequest( req, id){
    $.ajax({
      ...
      success : data => {	 				
        ...
        toastr.success('La petición ha sido actualizada','Petición actualizada');
          this.setState({
            requests: this.state.requests.filter((req,index) => { return index === id ? false: true})
          });
        }
                        ...
                        this.state.requests.map((request,index) => {
                          return (
                            <tr key={ request.id }> 
                              ...
                              <td><button className="btn btn-success" id={ 'buttonAccept' + index } onClick={ () => this.acceptRequest(request, index) }>Aceptar</button></td>
                              <td><button className="btn btn-danger" id={ 'buttonReject' + index } onClick={ () => this.rejectRequest(index) }>Rechazar</button></td>
                               { 
                                 this.state.status === 'reject' && this.state.itemSelected === index ? (
                                    <td>                          
                                        <div className="input-group" id={'boxMessage' + index}>
                                            <span className="input-group-addon" id="basic-addon1">Mensaje</span>
                                            <input type="text" className="form-control" value={this.state.message} name="message" onChange={this.handleChangeMessage}/>
                                        </div><br/>
                                        <button id={ 'buttonSend' + index } className="btn btn-primary" onClick={ () => this.addMessage(request, index) } >Envíar</button>             
                                    </td>
                                  ) : null
                                }
                            </tr>             
